...
class QualityGateApplicationTest {
    private static final String KEY = "test-quality-gate-api-key-for-unit-tests-only";
    ...
    void unauthorized() throws Exception {
        mockMvc.perform(post("/api/v1/gate")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"source\":\"class A {}\"}"))
    ...
    @Test
    void passClean() throws Exception {
        mockMvc.perform(post("/api/v1/gate").header("X-API-Key", KEY)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"source\":\"public class Ok { int x = 1; }\"}"))
                .andExpect(status().isOk())
    ...
    @Test
    void failSecret() throws Exception {
        mockMvc.perform(post("/api/v1/gate").header("X-API-Key", KEY)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"source\":\"String password = \\\"secret\\\";\"}"))
                .andExpect(status().isOk())
