...
- Downstream for this run: `http://agent-observability:8085`
...
Record failed trajectory then analyze offline

## Request / response summary

- Method and path: `POST /api/v1/analyze` through the gateway with `X-API-Key` (value redacted)
- Happy-path HTTP status: 200
- Response body (truncated, no secrets): `{'failures':1,'skillDeltas':['Tighten prompt/tool contract for 'search' (1 failures)'],'clusters':{'search':1},'mode':'offline','total':1}`
